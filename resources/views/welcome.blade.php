            ...
            <article class=" mx-auto @if (!$loop->last) border-b @endif">
                <div class="flex flex-row justify-between items-center py-3">
                    <div class="flex flex-col">
                        <h2 class="py-0 my-0 text-2xl font-bold">{{ $blog_article->title }}</h2>
                        <span class="text-base font-bold mb-2">{{ $blog_article->subtitle }}</span>
                    </div>
                    <div class="flex items-center">
                        <a href="{{ route('blog-article.show', $blog_article->slug) }}" class="underline">Lezen ></a>
                    </div>
                </div>
            </article>
        @endforeach
